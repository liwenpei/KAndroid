...
public class ClientSocket extends Thread{
    ...
    public void run() {
        ...
        try {
            ...
            LogUtil.debug(SOCKET_TAG, "client socket create successed");
            // 轮询发送消息列表中的数据
            while(true){
                // 判断是否要发送心跳包
                ...
                if(mSocket.isConnected()){
                    LogUtil.debug(SOCKET_TAG, "### client socket connected ###");
                    // 发送数据
                    if(!mSocket.isOutputShutdown() && !isMsgQueueEmpty()){
                        out.write(mMsgQueue.get(0).getBytes());
                        LogUtil.debug(SOCKET_TAG, "sned msg toServer: " + mMsgQueue.get(0));
                        LogUtil.debug(SOCKET_TAG, "## msg count : " + mMsgQueue.size());
                        // 将发送过的数据移除消息列表
                        mMsgQueue.remove(0);
                        mHeartTime = System.currentTimeMillis();
                    }
                    ...
                    // 重建连接
                    LogUtil.debug(SOCKET_TAG, "client socket disconnected");
            ...
            // TODO Auto-generated catch block
            e.printStackTrace();
            LogUtil.debug(SOCKET_TAG,e.getMessage());
            ...
            LogUtil.debug(SOCKET_TAG, "client socket close");
        }
    }
    ...
    protected void finalize() throws Throwable {
        ...
        LogUtil.debug(SOCKET_TAG, "client socket destory");
    }
