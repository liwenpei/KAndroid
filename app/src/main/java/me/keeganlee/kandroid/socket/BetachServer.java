...
public class BetachServer {
    ...
    public ByteBuffer response(SelectionKey key) throws IOException {
        SocketChannel sc = (SocketChannel) key.channel();
        ByteBuffer buffer = ByteBuffer.allocate(8);
        ByteBuffer resqBuff = null;
        try {
            LogUtil.debug("=============开始接收客户端数据================");
            ...
            if (length > 0) {
                ...
                // dataBuff = ByteBuffer.allocate(length);
                // int dataRnt = sc.read(dataBuff);
                // if(dataRnt != length){
                // LogUtil.debug("sorket 传送数据不一致");
                // resqBuff =
                // ByteBuffer.wrap(contactManager.getReponseErrData(BaseBetach.I4ANDROID_E_CMD_ERROR,"sorket 传送数据不一致"));
                // return resqBuff;
                // }
            }
            ...
            LogUtil.debug("接受到指令，指令code ： " + request);
            switch (request) {
                ...
                case BaseBetach.I4ANDROID_E_CMD_FILES_RESOURCE_IMPORT_REQ:
                    ...
                    HashMap<String, HashMap<String, String>> data = (HashMap<String, HashMap<String, String>>) AppConfig.Session
                            .get("data");
                    if (data != null && data.get(bUsReserve + "") != null) {
                ...
                case BaseBetach.I4ANDROID_E_CMD_FILES_RESOURCE_EXPORT_REQ:
                    fileManager = new BetachFileManager(context);
                    jsonStr = new String(dataBuff.array(),"utf-8");//ConvertUtil.byteTOString(dataBuff.array());
                    json = new JSONObject(jsonStr);
                    if (json != null) {
                        int rntId = json.getInt("return");
                        String msg = json.getString("reason");
...
/*			case BaseBetach.I4ANDROID_E_CMD_CONTACT_UPDATE_REQ:
                ...
                case BaseBetach.I4ANDROID_E_CMD_PHONE_CALL_IMPORT_REQ:
                    ...
                    betachCallManager.addCall(dataBuff);
...
//					        context.sendBroadcast(intent);
                ...
                case BaseBetach.I4ANDROID_E_CMD_MESSAGE_CHANGE_AUTHORITY_REQ:
                    ...
                    if (json != null && json.length() != 0) {
                        ...
                        if (rntId == 0) {
                            ...
                            if(arrJson != null && arrJson.length() > 0){
                                JSONObject jsonObj = arrJson.getJSONObject(0);
                                type = jsonObj.getInt("type"); // 1请求获取更改信息权限
                    ...
                    if (json != null) {
                        ...
                        if (rntId == 0) {
                            ...
                            if (arrJson.length() > 0) {
                                ...
                                type = obj.getInt("type");//1为获取应用图片缓存md5，参数为packageName ,,2 为获取图片库缩略图md5 参数为 filePath ,3 为其他文件MD5 参数为filePath
                                switch(type){
                                    case 1:
                ...
                case BaseBetach.I4ANDROID_E_CMD_SOCKET_LISTENER_REQ:
                    if(key.channel() != null && key.channel().isOpen()){
                        AppConfig.Session.put("SelectionKey", key);
                        //this.key = key;
                    ...
                    break;
                default:
                    LogUtil.debug("接受到的指令是错误的，指令code ： " + request);
